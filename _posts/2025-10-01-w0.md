...
1. Rubric override: The rubric is where the grouping happens and all this is about is just to add a reference answer to its state to layer on within the reward function it has something to compare it against. 
...
2. Reward Function: Uses state to access the reward function and does the pair-wise comparison.
```python
 def make_rf():
